...
import { MessageSquare, Pin, CheckCircle, Clock, Eye, Plus, Search, ArrowLeft } from 'lucide-react';
import { discussionForumAPI, courseAPI, teacherAPI } from '../../services/api';
...
const TeacherDiscussionForum = () => {
  ...
  const fetchForums = async () => {
    try {
      // First get the teacher by user ID
      const teacherResponse = await teacherAPI.getTeacherByUserId(user.id);
      if (!teacherResponse.data) {
        console.log('Teacher not found for user:', user.id);
        setLoading(false);
        return;
      }
      
      const teacherId = teacherResponse.data.teacherId;
      const coursesResponse = await courseAPI.getCoursesByTeacher(teacherId);
  ...
  const createQuestion = async () => {
    try {
      ...
      const teacherResponse = await teacherAPI.getTeacherByUserId(user.id);
      if (!teacherResponse.data) {
        alert('Teacher account not found. Please contact support.');
        return;
      }
      
      const teacherId = teacherResponse.data.teacherId;
      const coursesResponse = await courseAPI.getCoursesByTeacher(teacherId);
      console.log('Teacher courses:', coursesResponse.data);
      
      if (!coursesResponse.data || coursesResponse.data.length === 0) {
        alert('You need to create a course first before you can create discussion forums.');
        return;
      }
      
      const courseId = coursesResponse.data[0]?.courseId || coursesResponse.data[0]?.id;
      
      console.log('Creating forum with:', { teacherId, courseId, title: newQuestion.title });
        ...
        <>
          ...
          {filteredForums.length === 0 ? (
            <div className="card text-center py-12 text-gray-500">
              ...
              <p className="text-sm">Start a conversation by creating a discussion!</p>
            </div>
          ) : (
            <div className="space-y-4">
      ...
      {showNewQuestion && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="card max-w-lg w-full mx-4">
            <h2 className="text-xl font-bold text-gray-900 mb-4">Create Discussion</h2>
            <div className="space-y-4">
              <div>
...
export default TeacherDiscussionForum;